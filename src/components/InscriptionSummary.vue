...
const sendInscription = async (parents) => {
  ...
  if (apiData.files.length === 0) {
    showToast('Please select feeds', 'error')
    return
  }
  ...
  showToast('Creating direct order...', 'success')

  try {
    const files = createChildrenFilesPayload(apiData.files)
    ...
    showToast(`Direct Order created successfully: ${response.id}`, 'success')
    return true
  } catch (error) {
...
<template>
  <div class="flex items-center justify-end gap-x-12">
    <h5 class="text-2xl font-semibold">You are going to feed {{ apiData.parents.length }} pet</h5>
    <a
